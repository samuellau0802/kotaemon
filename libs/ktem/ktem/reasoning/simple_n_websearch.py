...
class FullQAWebSearchPipeline(FullQAPipeline):
    """Question answering with online search pipeline. Handle from question to answer"""
    ...
    def retrieve(
        ...
        #### add websearch
        web_docs, web_infos = web_search(message)
        docs += web_docs
        info += web_infos
    ...
    def get_info(cls) -> dict:
        return {
            "id": "Simple QA + Web Search",
            "name": "Simple QA + Web Search Agent",
            "description": (
                ...
                "context. It will also search the internet. After that it includes that context to generate the answer."
            ),
        }
